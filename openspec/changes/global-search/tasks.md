- [x] 1. 提取 `messages.vue` 中的静态聊天记录为 Vue data 数组，并通过 `v-for` 重构列表渲染。
- [x] 2. 在 `messages.vue` 添加搜索框交互逻辑（点击 icon 展开输入框）及基于 `computed` 的列表过滤功能。
- [x] 3. 新建 `pages/search/search.vue` 页面，实现顶部搜索栏、历史/热门搜索区域以及搜索结果瀑布流展示。
